...
The point is not that the content is better than what is on the internet. The
point is that the sidebar has no escape hatch: it structurally cannot become
YouTube.

## What it does

- **Detects agent state** through Claude Code hooks — not by scraping the
  terminal, so it does not break when the UI changes
- **Opens the sidebar** the moment Claude starts working, without ever stealing
  your keyboard focus
- **Plays a sound** the instant Claude needs input, via the `PermissionRequest`
  hook
- **Serves short cards**: AI news, engineering blogs, frontend and backend
  tips, senior interview questions, and output-based code quizzes
- **Filters by track** — Frontend, Node.js, Python, Java, Go, System Design,
  Misc — so a Go developer never reads Python cards
- **Focus mode** opens the same feed in a full editor tab when you want room
...
Open the folder in VS Code and press **F5** to launch a window with the
...
Worth knowing before you install anything that runs in your editor:
...
Cards are generated by Claude on a schedule, then **reviewed by hand before
anything is published**. An LLM-written quiz will occasionally ship a wrong
answer key, and a card that confidently teaches the wrong thing is worse than
...
npm run admin       # content review console, localhost only
npm run generate    # produce cards into the review queue
```

Design work is faster through `dev/preview.html`, which runs the real webview
bundle in a browser against the seed feed:

```bash
npm run build && python3 -m http.server 8791
# open http://localhost:8791/dev/preview.html
```
...
Enable Pages in the repo (**Settings → Pages → Source: GitHub Actions**), then
after each review session:

```bash
npm run publish             # commits data/feed.json and pushes
```

The [publish workflow](.github/workflows/publish-feed.yml) validates the feed —
non-empty, no duplicate ids, required fields present — and deploys it. Only
`data/feed.json` is committed; the unreviewed queue and rejection log stay
local, so nothing reaches readers without passing review.
